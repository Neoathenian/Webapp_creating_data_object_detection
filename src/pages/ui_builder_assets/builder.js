...
async () => {
  ...
  function updateApiKeySection(meta, { showSecret = false, secretValue = '' } = {}) {
    ...
    if (keyHint) keyHint.textContent = 'Keys are shown only once—store it securely.';
  ...
  if (btnGenerateKey) {
    btnGenerateKey.onclick = async () => {
      ...
      if (state.apiKeyMeta && state.apiKeyMeta.has_key) {
        const ok = window.confirm('Generate a new key? Existing integrations using the old key will stop working.');
        if (!ok) return;
      }
      btnGenerateKey.disabled = true;
      if (keyStatus) keyStatus.textContent = 'Generating key…';
      try {
        const res = await fetch('/builder/api-key', { method: 'POST' });
  ...
  if (btnDeleteKey) {
    btnDeleteKey.disabled = !(state.apiKeyMeta && state.apiKeyMeta.has_key);
    btnDeleteKey.onclick = async () => {
      if (btnDeleteKey.disabled || !(state.apiKeyMeta && state.apiKeyMeta.has_key)) return;
      const ok = window.confirm('Delete the current API key? Any requests using it will be rejected.');
      if (!ok) return;
      btnDeleteKey.disabled = true;
      if (keyStatus) keyStatus.textContent = 'Deleting key…';
      try {
        const res = await fetch('/builder/api-key', { method: 'DELETE' });
