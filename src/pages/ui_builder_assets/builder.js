...
async () => {
  ...
  function renderSecret({ showSecret = false, secretValue = '' } = {}) {
    ...
    if (showSecret && secretValue) {
      ...
      if (keyHint) keyHint.textContent = 'Keys are shown only once—store them securely.';
    }
  }
  ...
  async function refreshApiKeys({ silent = false } = {}) {
    ...
    try {
      const res = await fetch('/builder/api-key', { credentials: 'include' });
      if (res.status === 401) {
  ...
  if (keyList) {
    keyList.addEventListener('click', async (event) => {
      ...
      const confirmed = window.confirm('Delete this API key? Any requests using it will be rejected.');
      if (!confirmed) return;
      ...
      try {
        ...
        if (res.status === 401) {
          window.alert('Sign in to manage API keys.');
          btn.disabled = false;
